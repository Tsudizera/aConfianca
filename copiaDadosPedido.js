...
switch (true) {
  ...
  case link.includes("integracommerce.com.br/Order"): {
    ...
    for (const i of itens) {
      const umaConstanteComNomeRuim = i.querySelectorAll("td.pl15.valign-top");
      const obj = {
  ...
  case link.includes("americanasmarketplace.com.br/v3/pedidos/detalhes"): {
    ...
    const listaModelo = item.querySelectorAll(".p14-bold");
    const listaSku = item.querySelectorAll("article .mt-10 span:not(.mr-20)");
    const listaQtd = item.querySelectorAll("div.row.mt-25 div:first-child span");
    const listaItens = [];
  ...
  case link.includes("amazon.com.br/orders-v3/order"): {
    ...
    for (const i of itens) {
      const obj = {
        modelo: i.querySelector("a").innerText,
        sku: i.querySelectorAll(".product-name-column-word-wrap-break-all")[1].innerText,
        qtd: i.querySelectorAll("td")[4].innerText
      };
      listaItens.push(obj)
    }
  ...
  case link.includes("aconfianca.myvtex.com/admin/checkout/#/orders/"): {
    console.log("SITE");
    const doc = $("iframe", top.document).contentWindow.document;
    const pedido = $("span.order-id", doc).innerText;
    const nome = $("h4.card-subtitle.hidden-print", doc).innerText;
    const cpf = $("span[ng-if='::order.profile.document']", doc).innerText;
    const itens = $$(".stacked-item", doc);

    const listaItens = [];
    for (const i of itens) {
      const obj = {
        modelo: $("a", i).innerText,
        sku: $("span.ref", i).innerText,
        qtd: $("strong", i).innerText
      };
      listaItens.push(obj)
    }
